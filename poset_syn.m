...
function O=data_gen(n,d,min_num)
    O_1=zeros(n,d,d);
    O_o=zeros(n,d,d);
    for i=1:n
        M=randmtx(d,min_num);
...
function O1hat=step1clb(O1_o,O1_all,n,d,eps1)
    ...
    O1hat=zeros(d,d,d);
    for i=1:n
        j=length(find(O1_o(i,:,:)));
        counter(j,:,:)=counter(j,:,:)+O1_all(i,:,:);
    end
    for k=2:d
        for i=1:d
            for j=(i+1):d
...
%                 syms x y;
%                 [x,y]=solve(p*x+q*y==counter(k,i,j),p*y+q*x==counter(k,j,i));
